...
class DivergenceFreeVectorArray(VectorArray):
    ...
    def __init__(self, va, space, is_projected=True):
        ...
        `is_projected` indicates wether the projection `P V` has already been performed.
        """
        assert isinstance(space, DivergenceFreeSubSpace)
        assert va.space.dim == space.dim
        self._va = va
        self.space = space
        self.coordinate_space = va.space
    ...
    def dot(self, other):
        if other in self.space:
            return self.get_va().dot(other.get_va())
        else:
            raise NotImplementedError

    def pairwise_dot(self, other):
        if other in self.space:
            return self.get_va().pairwise_dot(other.get_va())
        else:
            raise NotImplementedError
    ...
    def l1_norm(self):
        return self.get_va().l1_norm()

    def l2_norm(self):
        return self.get_va().l2_norm()

    def l2_norm2(self):
        return self.get_va().l2_norm2()
...
class DivergenceFreeSubSpace(VectorSpace):
    ...
    which is fullfilled if and only if
    ...
    def zeros(self, count=1, reserve=0):
        return DivergenceFreeVectorArray(self.coordinate_space.zeros(count, reserve), self, is_projected=True)
    ...
    def project_onto_subspace(self, V, trans=False):
        print('PROJECT ONTO SUBSPACE!!!')
        assert V in self.coordinate_space
        from pymor.operators.block import BlockOperator
        if trans:
            rhs = BlockVectorArray([self.E.apply(V), self.G.source.zeros(len(V))])
            sps = BlockOperator([
                [self.E, self.G],
                [self.G.H, None]
            ])
            Vp = sps.apply_inverse(rhs).block(0)
        else:
